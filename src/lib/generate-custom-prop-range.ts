...
export default function generateCustomPropRange(
  ...
  const values = mapWithPrevious<CustomPropRangeDefinition, CssValue[]>(
    (previous, definition) => {
      if (Array.isArray(definition)) {
        return transformDefinition(definition)
      }

      const transformedDefinition = previous[previous.length - 1].map(
        ([value, unit]): CssValue => [value * definition, unit],
      )
